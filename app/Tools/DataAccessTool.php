...
use Prism\Prism\Tool as BaseTool; // Alias the base Tool class
...
class DataAccessTool extends BaseTool
{
    ...
    public function __invoke(
        ...
        if (!$user || !$team) {
            ...
            return json_encode([
                "error" =>
                    "User or team context not found. Cannot access data.",
            ]);
        }
        ...
        if ($query_type === "count_items" && !$item_type_for_count) {
            return json_encode([
                "error" =>
                    "The item_type_for_count parameter is required when query_type is count_items.",
            ]);
        }
        if (
            in_array($query_type, [
            ...
            return json_encode([
                "error" =>
                    "The identifier parameter is required for query_type: " .
                    $query_type,
            ]);
        }
        ...
        try {
            ...
            $result = [
                "error" =>
                    "An internal error occurred while fetching data: " .
                    $e->getMessage(),
            ];
        }
    ...
    private function listStudents($team, $filters)
    {
        ...
        return [
            ...
            "note" =>
                "Showing up to 20 students. Use filters or identifiers for specific searches.",
        ];
    }
    ...
    private function getStudentDetails($team, $identifier, $summarize = false)
    {
        ...
        if (!$student) {
            return [
                "error" =>
                    "Student not found matching identifier: " . $identifier,
            ];
        }
    ...
    private function listActivities($team, $filters)
    {
        ...
        return [
            ...
            "note" =>
                "Showing up to 15 activities. Use filters for specific searches.",
        ];
    }
    ...
    private function getActivityDetails($team, $identifier)
    {
        ...
        if (!$activity) {
            return [
                "error" =>
                    "Activity not found matching identifier: " . $identifier,
            ];
        }
