...
class Gradesheet extends Page implements HasTable
{
    ...
    public function table(Table $table): Table
    {
        ...
        foreach ($activities as $activity) {
            $columns[] = TextInputColumn::make("activity_{$activity->id}")
                ...
                ->type('number')
                ->rules(['nullable', 'numeric', 'min:0', "max:{$activity->total_points}"])
                ->state(function ($record) use ($activity) {
                    return $this->activityScores[$record->id][$activity->id] ?? null;
                })
                ->afterStateUpdated(function ($state, $record) use ($activity) {
                    $this->updateScore($record->id, $activity->id, $state);
                });
        }
        ...
        return $table
            ...
            ->actions([
                TableAction::make('edit_scores')
                    ...
                    ->modalWidth('lg')
                    ->slideOver()
                    ->form(function (User $record) use ($activities): array {
                        $schema = [];
                        ...
                        return [
                            Section::make('Activity Scores')
                                ->description(fn(User $record) => "Edit scores for {$record->name}")
                                ->schema($schema)
                                ->columns(1),
                        ];
                    })
                ...
                TableAction::make('view_final_grade')
                    ...
                    ->color('success')
                    ->modalHeading(fn(User $record) => "Final Grade for {$record->name}")
                    ->modalDescription('Detailed breakdown of the student\'s final grade based on activity scores.')
                    ->modalWidth('md')
                    ->action(function () {})
                    ->modalContent(function (User $record) use ($activities): HtmlString {
            ...
            ->modifyQueryUsing(function (Builder $query) use ($team) {
                $query->with(['teams' => function ($query) use ($team) {
                    $query->where('teams.id', $team->id);
                }]);
            });
    }
    ...
    protected function exportGrades()
    {
        ...
        foreach ($students as $student) {
            ...
            if ($this->showFinalGrades) {
                $finalGrade = $this->calculateFinalGrade($student->id, $activities);
                $row[] = $finalGrade !== 'N/A' ? str_replace('%', '', $finalGrade) : '';
            }
