        ...
        return [
            Action::make('submit')
                ->label(fn() => $this->activeOption === 'create'
                    ? 'Create Class'
                    : 'Join Class')
                ->icon(fn() => $this->activeOption === 'create'
    ...
    protected function joinExistingTeam(array $data): Model
    {
        ...
        if (!$team) {
            Notification::make()
                ...
                ->body('The provided class code is invalid. Please check and try again.')
                ->send();
    ...
    protected function createNewTeam(array $data): Model
    {
        ...
        try {
            ...
            Notification::make()
                ...
                ->body("Your class '{$data['name']}' has been created successfully.")
                ->send();

            // Redirect to the dashboard
            redirect()->to('/app/dashboard')->send();
